...
def get_multiplied_digits(number):
    ...
    if len(str_number) > 1:
        ...
        return first * get_multiplied_digits(int(str_number[1:])) # Таким образом вы умножите первую цифру числа на результат работы этой же функции с числом,но уже без первой цифры. пункт можно выполнить только тогда, когда длина str_number больше 1,иначе  не получиться взять срез str_number[1:].
    else:
        return number #Если же длина str_number не больше 1, тогда вернуть оставшуюся цифру first


print(get_multiplied_digits(57))
